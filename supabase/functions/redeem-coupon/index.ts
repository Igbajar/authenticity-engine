...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (couponType === "trial_extension") {
      ...
      // Grant free subscription for `value` days on specified tier
      ...
      // Add scans to current subscription
      const { data: sub } = await adminClient
        .from("user_subscriptions")
        .select("*")
        .eq("user_id", user.id)
      ...
      // Discount would be applied at Stripe checkout; store in redemption for reference
    }
