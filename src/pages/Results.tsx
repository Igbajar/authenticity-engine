...
import { 
  ...
  Brain, ExternalLink, Copy, Loader2, ChevronDown, ChevronUp, BookOpen
} from "lucide-react";
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="rounded-2xl border border-border bg-card">
              <div className="p-6 border-b border-border">
                <h3 className="font-medium text-foreground">Source Matches ({matches.length})</h3>
                <p className="text-sm text-muted-foreground">Text passages found in other sources</p>
              </div>
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <span className={`text-sm font-medium ${
                              match.similarity_percentage > 50 ? "text-destructive" : "text-warning"
                            }`}>
                              {match.similarity_percentage}% match
                            </span>
                            <span className="text-xs text-muted-foreground">•</span>
                            <span className="text-sm text-muted-foreground truncate">
                              {match.source_title || "Unknown Source"}
                            </span>
                          </div>
                      ...
                      {expandedMatches.has(match.id) && (
                        <div className="mt-4 p-4 bg-muted/50 rounded-lg">
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <button 
                              onClick={() => copyToClipboard(match.matched_text)}
                              className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-1"
                            >
                              <Copy className="w-3 h-3" /> Copy
                            </button>
                          </div>
                          <p className="text-sm text-foreground">{match.matched_text}</p>
                        </div>
                      )}
                    </div>
                  ))}
